...
export default function MyButtons({ Complete, pending, all}) {
    ...
    function handleDeleteTask(id) {
        ...
        setTaskList(updatedTasks);
        localStorage.setItem("tasks", JSON.stringify(updatedTasks)); // will return an array without the task 
    }

    

    let filteringMethod = (task) => true;

    if (pending == true) {
        filteringMethod = (task) => task.completed === false;
    }else if (Complete == true) {
        filteringMethod = (task) => task.completed === true;
    }else if(all == true){
        filteringMethod = (task) => true;
    }
    ...
    let TasksDiv = taskList.filter(filteringMethod).map((task) => {
        return(
            // the big div 
            <div 
